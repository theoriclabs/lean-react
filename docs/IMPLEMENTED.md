...
- Live data: `useChannel` (WebSocket Channels, one socket per origin) and `useStream` (SSE fallback). There is no shared query cache, optimistic mutation framework, hydration qualification, or React Server Components integration. React server rendering is exercised as a workload check.
