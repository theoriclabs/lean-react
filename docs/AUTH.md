...
`Service.Config.sessionCacheTtlMs` (`LEANAPP_SESSION_CACHE_TTL_MS` in the executables; default 0 = off, 30 s recommended) keeps resolved sessions in process memory, keyed by the token digest and guarded by the service's small mutex, never the database queue. On the serialized hosts a hit skips only the two point reads of the authentication step and the application callback still runs under `withConnection`; on a lanes host (`Auth.Host.createWith`, the café by default) a hit means the request reaches its handler without touching any connection, and a miss resolves the session with one read on a pooled reader (`Service.resolve`), caching it only if no invalidation happened while the row was read. Hits still check CSRF and session expiry. Because exactly one Lean process serves an instance, every event that changes session validity happens here and invalidates immediately: logout, targeted revocation, sign-out-everywhere, password change, `setAccess` and eviction by the session cap. The TTL bounds staleness only for defence in depth and for `lastSeenAt`, which a hit does not refresh. `sessionCacheMax` (default 10,000 entries) clears the cache when full. `Service.cacheStats` reports hits, misses, invalidations and size. `npm run test:auth` ends with a load check of 10,000 authenticated calls with the cache off and on and prints the wall-time reduction and, from `/internal/metrics`, the summed DB-queue time (wait plus held): on a laptop about 45–55 % less wall time and about 85 % less queue time, because the authentication reads and their query-log writes were most of the writer's work per call.
...
Every request is logged as one JSON line without bodies, tokens or usernames; see [Operations](HOSTING.md#lean-backend-operations) for the fields, `X-Request-Id` handling and the loopback-only `GET /internal/metrics`.
...
`Host.listen` authenticates at the upgrade, not from a frame. The gateway already checked `Origin` and that a session cookie is present; Lean does not trust that work. `onUpgrade` reads `leanapp_session` / `__Host-leanapp_session` from the forwarded `Cookie` header, resolves it through `Auth.Service` (the same path as HTTP, including the session cache), and checks `Origin` again. A missing or invalid cookie is `401`. A forged or missing `Origin` is `403`. The socket's `RequestContext` is fixed at that point and is never taken from a later frame.

The first data frame must be `hello {csrf}` equal to that session's CSRF token, within 5 seconds; otherwise the socket closes `1008`. Logout, targeted revocation, sign-out-everywhere, a password change and `setAccess` notify `Auth.Service.onInvalidation`; matching sockets receive `closed` with reason `session` and are torn down. Per-subscription `seq` starts at 1 after `subscribed`. Inbound `send` is rate-limited per socket (`inboundPerSecond`, default 10). Queue overflow closes that subscription with `closed overflow`, not the socket. Optional `call` frames dispatch through the host's `onCall` hook and reply with the same JSON body HTTP would have returned.
