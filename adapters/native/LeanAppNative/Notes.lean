...
def host (auth : Auth.Service) (origin : String) (development : Bool := false) : IO Auth.Host := do
  ...
  let .ok server := Server.create template codecs { maxBodyBytes := 4096, errorStatuses := statuses }
