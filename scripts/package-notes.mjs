...
const out = await mkdtemp(resolve(parent, 'private-notes-'));
const files = [];
...
// `engine` includes `engine/gateway`, the public process module serve-notes.mjs configures.
for (const name of ['engine', 'examples/ordering', 'examples/notes', 'examples/security', 'tests/security', 'adapters/native/LeanAppNative', 'adapters/native/bindings'])
  await copy(resolve(root, name), resolve(out, 'leanreact', name), true);
for (const name of ['adapters/native/lakefile.lean', 'adapters/native/lean-toolchain', 'adapters/native/NotesMain.lean', 'adapters/native/NotesChecks.lean',
...
await copy(resolve(root, 'deploy/notes/Dockerfile'), resolve(out, 'Dockerfile'));
