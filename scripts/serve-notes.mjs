...
const backendPort = Number(process.env.LEANAPP_BACKEND_PORT ?? 4191);
...
const binary = process.env.LEANAPP_NOTES_BINARY ?? resolve(root, 'adapters/native/.lake/build/bin/leanapp_notes');
const assets = {};
for (const [path, file, type] of [['/', 'index.html', 'text/html'], ['/main.js', 'main.js', 'text/javascript'], ['/style.css', 'style.css', 'text/css'], ['/evidence.json', 'evidence.json', 'application/json'], ['/spec.lean', 'spec.lean', 'text/plain'], ['/model.lean', 'model.lean', 'text/plain']])
  assets[path] = { bytes: await readFile(resolve(root, 'examples/dist-notes', file)), type };
...
if (!ready) { shutdown(1); } else {
  ...
  server = createServer(async (req, res) => {
    ...
    if (!/^\/(auth\/[a-z]+|api\/notes\/(list|lookup|search|count|export|lab)|health\/ready)$/.test(path) || !['GET', 'POST'].includes(req.method)) { send(404); return; }
  ...
  server.listen(port, development ? '127.0.0.1' : '0.0.0.0', () => console.log(`Private Notes listening on port ${port}`));
}
